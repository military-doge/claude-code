...
1. 打开终端，运行下面命令（`/home/user/claude-code` 是仓库默认目录，若你的仓库放在别处请**替换成你解压/克隆后的实际路径**）。它在 `~/.bun/bin` 目录下生成一个名为 `claude` 的软链接指向本仓库；该目录由安装 bun 时创建，并已自动加入 PATH。
2. **重新打开**一个终端，在任意目录直接输入 `claude` 并回车，即可启动会话。

```bash
ln -sf /home/user/claude-code/bin/claude ~/.bun/bin/claude
```
